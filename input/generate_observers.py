...
# For each longitude, create an observer at each altitude and time
...
colnames = 'lon lat altitude time time_mjd time_tdb'.split()

table = Table(rows=table, names=colnames)
table['lon'].format = "%20.15f"
table['lat'].format = "%20.15f"

table.write('observers.txt', format='ascii.csv')
